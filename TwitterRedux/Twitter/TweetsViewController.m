...
@property (nonatomic) TweetsViewStyle tweetsStyle;
...
- (id)init {
    if (self = [super init]) {
        self.tweetsStyle = TweetsViewStyleHome;
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    if (self.tweetsStyle == TweetsViewStyleHome) {
...
- (void)reloadTweets {
    if ([self.reloadLock tryLock]) {
        ...
        } else if (self.tweetsStyle == TweetsViewStyleMentions) {
    ...
    if ([self.reloadLock tryLock]) {
        ...
        if (self.tweetsStyle == TweetsViewStyleHome) {
            [[TwitterClient sharedInstance] homeTimelineWithParams:@{@"max_id":maxId} completion:^(NSArray *tweets, NSError *error) {
                NSMutableArray *newTweets = [[NSMutableArray alloc] initWithArray:tweets];
            ...
            [[TwitterClient sharedInstance] mentionsTimelineWithParams:@{@"max_id":maxId} completion:^(NSArray *tweets, NSError *error) {
                NSMutableArray *newTweets = [[NSMutableArray alloc] initWithArray:tweets];
                [newTweets removeObjectAtIndex:0];
                if (newTweets.count > 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:cell action:@selector(onProfileImageTap:)];
